...
const LiveHistoryTable = ({ history, translations, idioma }) => {
  const { unit } = useUnit();
  ...
  const latestHistory = history.slice(0, 100);
          ...
          <tbody>
            {latestHistory.map((item, index) => (
              ...
              >
                ...
                <td className="p-1 text-center">{convertWindSpeed(item.avg_speed_kt, unit).toFixed(unit === 'beaufort' ? 0 : 1)} {unitLabel}</td>
                <td className="p-1 text-center">{convertWindSpeed(item.gust_speed_kt, unit).toFixed(unit === 'beaufort' ? 0 : 1)} {unitLabel}</td>
