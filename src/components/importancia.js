...
import esimg from '/public/importancia/es.png';
import itimg from '/public/importancia/it.png';
import enimg from '/public/importancia/en.png';
...
const EmpresaOnline = ({ idioma }) => {
  ...
  const { title, parraf1, parraf2, parraf3, parraf4, img } = contenido[idioma];
    ...
    <div className="bg-blue-900 text-white p-8 mt-4 mb-4">
      <div data-aos="fade-up">
        ...
        <img
          ...
          onClick={() => setLightboxController({ toggler: !lightboxController.toggler, slide: 1 })}
          style={{ display: 'block', margin: '0 auto' }}
        />
        <p className="mb-4">{parraf1}</p>
        <p className="mb-4">{parraf2}</p>
        <p className="mb-4">{parraf3}</p>
        <p className="font-semibold mb-2">{parraf4}</p>
      </div>
      <FsLightbox
